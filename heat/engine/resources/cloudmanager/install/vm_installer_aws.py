...
class aws_installer(object):
    ...
    def create_vm_test(self, image_id, instance_type, inter):
        ...
        interfaces = boto.ec2.networkinterface.NetworkInterfaceCollection(
            interface)
        instance = self.ec2_conn.run_instances(image_id,
    ...
    def create_vm(self, image_name, instance_type, interfaces_list):
        if len(interfaces_list) == 4:
            interface1 = boto.ec2.networkinterface.NetworkInterfaceSpecification(
                subnet_id=interfaces_list[0].subnet_id,
                private_ip_address=interfaces_list[0].private_ip_address,
                device_index=0)
            interface2 = boto.ec2.networkinterface.NetworkInterfaceSpecification(
                subnet_id=interfaces_list[1].subnet_id,
                private_ip_address=interfaces_list[1].private_ip_address,
                device_index=1)
            interface3 = boto.ec2.networkinterface.NetworkInterfaceSpecification(
                subnet_id=interfaces_list[2].subnet_id,
                private_ip_address=interfaces_list[2].private_ip_address,
                device_index=2)
            interface4 = boto.ec2.networkinterface.NetworkInterfaceSpecification(
                subnet_id=interfaces_list[3].subnet_id,
    ...
    def associate_address(self, instance, dev_no=0):
        ...
        for loop in range(100):
            try:
                loop = loop + 1
                self.ec2_conn.associate_address(public_ip=eip.public_ip,
                                                allocation_id=eip.allocation_id,
                                                network_interface_id=
                                                instance.interfaces[dev_no].id)
            except:
                print "ERROR"
                time.sleep(3)
                continue
    ...
    def disable_sdcheck(self, instance_id):
        return self.ec2_conn.modify_instance_attribute(instance_id,
                                                       "sourceDestCheck",
                                                       "false")
